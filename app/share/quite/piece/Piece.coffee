...
class Piece
  ...
  insertNext: (pieces) ->
    if not (pieces instanceof Array)
      pieces = [pieces]
    for piece in pieces
      @elmt.parentNode.insertBefore piece.elmt, @elmt.nextSibling

  insertAt: (pieces, position) ->
    if not (pieces instanceof Array)
      pieces = [pieces]
    for piece, index in pieces
      @elmt.insertBefore piece.elmt, @elmt.children[position + index]
    @

  h: (attr, elmtAttr) ->
    attr || attr='D'
    @elmt.setAttribute('data-holderid', attr)
    if elmtAttr isnt undefined
      @elmt.setAttribute('data-attr', elmtAttr)
